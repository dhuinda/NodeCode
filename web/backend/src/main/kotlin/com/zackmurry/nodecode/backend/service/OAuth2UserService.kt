...
import com.zackmurry.nodecode.backend.security.OAuth2UserInfo
import com.zackmurry.nodecode.backend.security.OAuth2UserInfoFactory
...
class OAuth2UserService(private val userService: UserService) : DefaultOAuth2UserService() {
    ...
    private fun registerNewUser(oAuth2UserRequest: OAuth2UserRequest, oAuth2UserInfo: OAuth2UserInfo): NodecodeUser {
        val username = oAuth2UserInfo.getUsername()
            ?: throw OAuth2AuthenticationProcessingException("Username not found from OAuth2 provider")
        val user = NodecodeUser(
            username,
